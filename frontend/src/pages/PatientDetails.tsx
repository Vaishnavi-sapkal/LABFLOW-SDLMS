...
import { CalendarDays, CreditCard, Download, FlaskConical, UserRound } from 'lucide-react';
...
import { getPatientDetails, getPatientReport, type Patient360Data } from '../api/patients';
...
export function PatientDetails() {
  ...
  const viewReport = async (reportId: string) => {
    if (!id) return;
    setReportLoadingId(reportId); setReportError('');
    try { setSelectedReport(await getPatientReport(id, reportId)); }
    catch (requestError) { setReportError(requestError instanceof Error ? requestError.message : 'Unable to open this report.'); }
    finally { setReportLoadingId(null); }
  };
  return <PageContainer>
    ...
    <section className="card mt-4 p-4 sm:p-5">
      ...
      {tab === 'Results & Reports' && <div className="grid gap-6"><div><h2 className="mb-3 text-base font-semibold">Results</h2><TableOrEmpty empty="No results found." columns={['Sample', 'Status', 'Remarks']} rows={(data.results ?? []).map((result) => <tr key={result._id}><DataCell>{result.sampleId}</DataCell><DataCell><StatusBadge tone="info">{result.status}</StatusBadge></DataCell><DataCell>{result.remarks ?? '-'}</DataCell></tr>)} /></div><div><h2 className="mb-3 text-base font-semibold">Reports</h2><TableOrEmpty empty="No reports found." columns={['Report', 'Test', 'Date', 'Action']} rows={(data.reports ?? []).map((report) => <tr key={report._id}><DataCell className="font-semibold">{report.reportNo}</DataCell><DataCell>{report.testName}</DataCell><DataCell>{date(report.reportDate)}</DataCell><DataCell><Button disabled={reportLoadingId === report._id} icon={<Download size={14} />} onClick={() => void viewReport(report._id)} size="sm" variant="outline">{reportLoadingId === report._id ? 'Opening...' : 'View Report'}</Button></DataCell></tr>)} />{reportError && <p className="mt-3 text-sm text-danger">{reportError}</p>}{selectedReport && <section className="mt-6"><div className="mb-3 flex items-center justify-between gap-3"><div><h3 className="text-base font-semibold">{selectedReport.testName}</h3><p className="text-sm text-ink-muted">{selectedReport.reportNo}</p></div><Button onClick={() => setSelectedReport(null)} size="sm" variant="outline">Close report</Button></div><ReportPreview doctor={selectedReport.doctorName} patient={`${selectedReport.patientName}${selectedReport.patientAge !== undefined ? ` | ${selectedReport.patientAge} yrs` : ''}${selectedReport.patientGender ? ` | ${selectedReport.patientGender}` : ''}`} reportDate={date(selectedReport.reportDate)} reportId={selectedReport.reportNo} sampleId={selectedReport.sampleId} tests={selectedReport.values.map((value) => ({ parameter: value.parameterName, value: value.value === null ? '-' : String(value.value), unit: value.unit, range: `${value.referenceMin} - ${value.referenceMax}`, flag: value.flag }))} /></section>}</div></div>}
